...
interface OriginalFilesSectionProps {
  searchQuery: string;
...
function OriginalFilesSection({
  searchQuery,
  onPreview,
  formatDateTime,
}: OriginalFilesSectionProps) {
  const [sortBy, setSortBy] = useState<string>('uploaded_at');
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc');
  const [page, setPage] = useState(1);

  const { data: response, isLoading } = useQuery({
    queryKey: ['search-original-files', searchQuery],
    queryFn: async () => {
  ...
  const sortedData = useMemo(() => {
    ...
    const sorted = [...data];
    sorted.sort((a, b) => {
      const aVal = a[sortBy as keyof UploadedFile];
      const bVal = b[sortBy as keyof UploadedFile];
...
export default memo(OriginalFilesSection);
